...
# NOTE: Temporarily disable DNSProxy to avoid creating Qt objects on worker thread
# (this helps diagnose the UI freeze). Re-enable only after DNSProxy is rewritten
# to avoid any Qt usage from background threads.
DNSProxy = None
# If you later want the original behavior, revert to:
# try:
...
def start_firewall(db_path=None, filter_str=None):
    ...
    # spawn the small helper thread and return immediately
...
def sync_blocked_ips(db_path=None):
    ...
    # Ensure table schema is up-to-date before we manipulate it
    ensure_blocked_ips_schema(db_path)

    """Resolve all domains in blocked_domains into blocked_ips; rebuild blocked_ips table."""
    try:
        conn = sqlite3.connect(db_path)
        cur = conn.cursor()
        cur.execute("CREATE TABLE IF NOT EXISTS blocked_domains(domain TEXT UNIQUE)")
        # Ensure base blocked_ips table exists (it should after ensure_blocked_ips_schema)
...
def remove_blocked_domain(domain: str, db_path=None):
    db_path = db_path or DEFAULT_DB
    os.makedirs(os.path.dirname(db_path), exist_ok=True)
    """Remove domain from blocked_domains table."""
    try:
        conn = sqlite3.connect(db_path)
        cur = conn.cursor()
...
class FirewallThread(threading.Thread):
    def __init__(self, db_path=None, filter_str=None):
        ...
        # capture outbound HTTP/HTTPS and QUIC (UDP/443)
        self.filter_str = filter_str or "outbound and (tcp.DstPort == 80 or tcp.DstPort == 443 or udp.DstPort == 443)"
        ...
        if DNSProxy is not None:
            try:
                self.dns_proxy = DNSProxy(bind_addr=("0.0.0.0", 53), upstream=("8.8.8.8", 53), db_path=self.db_path)
            except Exception as e:
            ...
            # Protect gateway/DNS/local addresses from being auto-blocked
            self._critical_ips = set(["127.0.0.1", "0.0.0.0"])

            # Try to detect default gateway (optional; requires netifaces package)
            try:
                import netifaces
                gw_info = netifaces.gateways().get('default', {}).get(netifaces.AF_INET)
                if gw_info:
                    self._critical_ips.add(gw_info[0])
            except Exception:
                # netifaces not installed or detection failed -> continue safely
                pass

            # Protect common public DNS resolvers too
            for _dns in ("8.8.8.8", "1.1.1.1", "9.9.9.9"):
                self._critical_ips.add(_dns)
            # -----------------------------------------------------------
    ...
    def run(self):
        ...
        try:
                        ...
                        if host:
                            for _, app_name, pattern, ip_range, proto in self.app_signatures:
                                if pattern and fnmatch.fnmatch(host, pattern):
                                    print(f"[APPBLOCK] Matched app '{app_name}' via {host}")
                                    domain_hit = pattern  # treat as blocked
                                    break
            ...
            # WinDivert open error or other top-level failures
